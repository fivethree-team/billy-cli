...
export class BillyCLI {
    ...
    async create(@param(appOptions) app, @context() context: Context) {
        if (!(await this.exists(context.workingDirectory + '/' + app))) {
            ...
            await this.exec(`git clone https://github.com/fivethree-team/billy-app.git ${app}`);
            ...
            console.log('pascalcase', app, (await this.camelcase(app, true)));
            ...
            await this.exec(`rm -rf ${context.workingDirectory + '/'}${app}/package-lock.json && npm install --prefix ${context.workingDirectory + '/'}${app}/`);
            console.log('Doing an initial build to see if everything is working. 🛠`')
            await this.exec(`${context.workingDirectory + '/'}${app}/node_modules/.bin/tsc -p ${context.workingDirectory + '/'}${app}`);
    ...
    async create_plugin(@param(pluginOptions) plugin: string, @context() context: Context) {
        ...
        if (!this.exists(plugin)) {
            ...
            console.log('Installing dependencies, this might take a while...⏳')
            await this.exec(`rm -rf ${context.workingDirectory + '/'}${plugin}/package-lock.json && npm install --prefix ${context.workingDirectory + '/'}${plugin}/`);
            console.log('Doing an initial build to see if everything is working. 🛠`')
            await this.exec(`${context.workingDirectory + '/'}${plugin}/node_modules/.bin/tsc -p ${context.workingDirectory + '/'}${plugin}`);
