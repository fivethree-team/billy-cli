...
export class BillyCLI {
  ...
  @Hook(onStart)
  async onStart(@param(appOptions) app, @context() context: Context) {
    console.log(app);
  ...
  async app(@param(appOptions) app, @context() context: Context) {
    ...
    await this.exec(
      `git clone https://github.com/fivethree-team/billy-app.git ${app}`,
      true
    );
    const packageJSON = await this.parseJSON(
      `${context.workingDirectory + "/"}${app}/package.json`
    );
    ...
    const contents = text.replace(
      "ExampleApplication",
      await this.camelcase(app, true)
    );
    await this.writeFile(app + "/src/index.ts", contents);

    console.log("Installing dependencies, this might take a while...⏳");
    await this.exec(
      `rm -rf ${context.workingDirectory +
        "/"}${app}/package-lock.json && npm install --prefix ${context.workingDirectory +
        "/"}${app}/`,
      true
    );
    ...
    await this.exec(
      `${context.workingDirectory +
        "/"}${app}/node_modules/.bin/tsc -p ${context.workingDirectory +
        "/"}${app}`,
      true
    );
  ...
  async plugin(
    @param(pluginOptions) name: string,
    @context() context: Context
  ) {
    console.log(`Ok, your plugins's name will be ${name}!`);
    console.log(`Cloning plugin repository⬇`);
    await this.exec(
      `git clone https://github.com/fivethree-team/billy-plugin.git ${name}`,
      true
    );
    const packageJSON = this.parseJSON(
      `${context.workingDirectory + "/"}${name}/package.json`
    );
    ...
    await this.exec(
      `rm -rf ${context.workingDirectory +
        "/"}${name}/package-lock.json && npm install --prefix ${context.workingDirectory +
        "/"}${name}/`,
      true
    );
    console.log("Doing an initial build to see if everything is working. 🛠`");
    await this.exec(
      `${context.workingDirectory +
        "/"}${name}/node_modules/.bin/tsc -p ${context.workingDirectory +
        "/"}${name}`,
      true
    );
  ...
  async build(@context() context: Context) {
    if (await this.billy(context.workingDirectory)) {
      await this.exec(
        `node_modules/.bin/tsc -p ${context.workingDirectory}`,
        true
      );
    } else {
